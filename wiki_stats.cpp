...
class WikiGraph{;
private:
    unsigned int number_of_articles;
    unsigned int number_of_links;
    ofstream wiki;
    vector<string> string;
    vector<unsighed int> sizes;
    vector<unsighed int> links;
    vector<unsighed int> redirect;
    vector<unsighed int> offset;


public:
    WikiGraph(){
        std::cout<<"Загружаю граф из файла: wiki_small.txt ";
        number_of_articles=0;
        number_of_links=0;
        wiki.open("/home/student/andrey543/wiki_stats","r");
        wiki>>number_of_articles>>number_of_links;
